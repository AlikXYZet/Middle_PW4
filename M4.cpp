...
int main()
{
	...
	// Очистка массива (Удаление всех элементов вектора)
	// P.S.: Получаем массив из 4-х ПУСТЫХ элементов ( v.capacity() = 4 )
	v.clear();

	// Уменьшаение количества используемой памяти за счёт освобождения неиспользованной
	// P.S.: Получаем пустой вектор без элементов
	v.shrink_to_fit();

	// Удаление вектора (вызов деструктора)
	// P.S: При вызове нет смысла предварительно вызывать .clear() и .shrink_to_fit()
	//v.~vector();
